...
def data_loss_neg(label_code, label, train_b, train_label, B, L, indicator, gamma=0.2, sigma=0.2, epsilon = 1e-5):

    code_length = label_code.shape[1]
    logit = train_b.mm(label_code.t())

    mulogit = train_b.mm(B.t()) * indicator.view(1, -1)
    negative = (train_label.mm(L.t()) < 1).float() * indicator.view(1, -1)

    max_item = (torch.cat((logit, mulogit), dim=1)).max(1)[0]
    logit = logit - max_item.view(-1, 1)
    mulogit = mulogit - max_item.view(-1, 1)
    sim_label = (train_label.mm(label.t()) > 0).float()
    our_logit = torch.exp((logit - sigma * code_length) * gamma) * sim_label
    mu_logit = (torch.exp(mulogit * gamma) * negative).sum(1).view(-1, 1) + our_logit
    loss = - (((torch.log(our_logit / (mu_logit + epsilon) + epsilon) * sim_label).sum(1) / (
...
def PSLDH_Algo(code_length):
    ...
    indicator = torch.zeros(num_train).cuda()
            ...
            for epoch in range(10):
                ...
                for iter, traindata in enumerate(train_loader, 0):
                    ...
                    loss = data_loss(label_code, labels, hash_out, train_label, weighted[batch_ind], gamma=0.2,
                                     sigma=0.2)
                    if (zz != 0) or (xx != 0):
                        loss1 = data_loss_neg(label_code, labels, hash_out, train_label, torch.tensor(B).float().cuda(), train_labels.cuda(), indicator, gamma=0.2,
                                     sigma=0.2)
                        loss += loss1
                        epoch_loss_n += loss1.item()
                ...
                print('[Train Phase][Epoch: %3d/%3d][Loss_i: %3.5f, Loss_e: %3.5f, Loss_r: %3.5f, Loss_n: %3.5f]' %
                      (epoch + 1, epochs, epoch_loss / len(train_loader), epoch_loss_e / len(train_loader),
                       epoch_loss_r / len(train_loader), epoch_loss_n / len(train_loader)))
